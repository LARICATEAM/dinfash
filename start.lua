...
print("::dinfash::")
local RunBot = io.open("dinfash", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/dinfash
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr dinfash.lua
wget "https://raw.githubusercontent.com/LARICATEAM/dinfash/master/dinfash.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./dinfash.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S dinfash -X kill
screen -S dinfash ./dinfash
done
]])
RunTs:close()
end
